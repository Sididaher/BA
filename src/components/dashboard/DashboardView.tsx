  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <div className="max-w-md mx-auto px-5 pt-5 pb-28 space-y-6">
        ...
        {/* Welcome hero (countdown + progress) */}
        <WelcomeHero
        ...
        {/* Stats */}
        <div>
          <SectionRow title="Mes statistiques" />
          <div className="grid grid-cols-2 gap-3">
            <StatCard label="Leçons terminées" value={stats.completedLessons} icon={<CheckCircle2Icon size={20} className="text-green-600" />}  color="green"  />
            <StatCard label="En cours"          value={stats.inProgress}      icon={<PlayCircleIcon    size={20} className="text-primary"   />}  color="blue"   />
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {QUICK_ACTIONS.map(({ href, Icon, label, bg, fg }) => (
              <Link
                key={href}
                href={href}
              ...
              >
                ...
                <span className="text-[10px] font-semibold text-muted text-center leading-tight">{label}</span>
              </Link>
            ))}
          </div>
        </div>
